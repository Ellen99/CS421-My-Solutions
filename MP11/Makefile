...
$(PROB)-skeleton.ml: $(STUDENTSRC)
	$(CP) $(STUDENTSRC) $(PROB)-skeleton.ml

$(PROB).pdf: ../$(PROB).tex
	(cd ..; $(LATEX) $(PROB).tex; $(LATEX) $(PROB).tex)
	$(CP) ../$(PROB).pdf .

DISTFILES_SOURCE=pre-rubric.c tests Makefile $(MODULE_COMMON).ml $(INTERACTIVE_EXE).ml $(OBJLANG)parse.mly $(OBJLANG)lex.mll .ocamlinit $(PROB).pdf
...
DISTFILES_OTHER=README $(PROB).ml $(PROB)-skeleton.ml
...
dist: $(GRADER_NAME) $(DISTFILES)
	...
	$(CP) $(TESTNAME).ml $(TESTNAME)/$(TESTNAME)-skeleton.ml
